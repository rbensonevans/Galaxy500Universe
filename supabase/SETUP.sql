...
as $fn$
declare uid uuid := auth.uid(); w public.wallets; pending numeric(20,2);
begin
  if uid is null then raise exception 'Not authenticated'; end if;
  select * into w from public.wallets where user_id = uid for update;
  if not found then raise exception 'No wallet yet'; end if;
  pending := round(w.savings_balance * w.savings_rate * (extract(epoch from (now() - w.savings_accrued_at)) / 31536000.0), 2);
  if pending > 0 then
    update public.wallets set savings_balance = savings_balance + pending, savings_accrued_at = now()
      where user_id = uid returning * into w;
    insert into public.bank_transactions (user_id, amount, kind) values (uid, pending, 'interest');
  end if;
  return w;
end;
$fn$;
...
begin
  ...
  update public.wallets set balance = balance - amt, savings_balance = savings_balance + amt
    where user_id = uid returning * into w;
  insert into public.wallet_transactions (user_id, amount, kind, memo) values (uid, -amt, 'bank_deposit', 'Deposit to Crypto Bank');
  insert into public.bank_transactions (user_id, amount, kind) values (uid, amt, 'deposit');
  return w;
end;
$fn$;
...
begin
  ...
  update public.wallets set savings_balance = savings_balance - amt, balance = balance + amt
    where user_id = uid returning * into w;
  insert into public.wallet_transactions (user_id, amount, kind, memo) values (uid, amt, 'bank_withdraw', 'Withdrawal from Crypto Bank');
  insert into public.bank_transactions (user_id, amount, kind) values (uid, -amt, 'withdraw');
  return w;
end;
$fn$;
